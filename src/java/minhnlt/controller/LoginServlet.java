...
import minhnlt.tblUser.UserDAO;
...
public class LoginServlet extends HttpServlet {
    ...
    private final String LOAD_HOME_PAGE_SERVLET = "LoadHomePageServlet";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(checkLogin){
                UserDTO user = userdao.getDto();
                HttpSession session = request.getSession();
                session.setAttribute("USER", user);
                if(user.getRoleName().equals("admin")){
                    url = ADMIN_PAGE;
                }else if(user.getRoleName().equals("user")){
                    url = LOAD_HOME_PAGE_SERVLET;
                }
                
            }
        } catch (SQLException ex) {
            log("LoginServlet_SQLException" + ex.toString());
        } catch (NamingException ex) {
            log("LoginServlet_NamingException" + ex.toString());
        }finally{
